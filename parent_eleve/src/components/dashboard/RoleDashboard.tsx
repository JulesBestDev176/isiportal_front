...
export const RoleDashboard: React.FC = () => {
  ...
  const roleConfig = roleConfigurations[user.role as keyof typeof roleConfigurations];
  
  if (!roleConfig) {
    return (
      <div className="p-8 text-center">
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-6">
          <p className="text-orange-800">Rôle non reconnu: {user.role}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            ...
            >
              Bienvenue, {roleLabels[user.role] || user.role} !
            </motion.h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <RoleStats role={user.role} delay={0.4} />
        
        {/* Actions recommandées */}
        <RecommendedActions role={user.role} delay={0.5} />
